...
Type
  ...
  TSaS = class (TObject)
    ...
    procedure PPrChoice(M1:TMemo;btn:TButton);
    procedure PQuickSort(M1:TMemo);
    procedure PDSearch(E4:TEdit;M1:TMemo;btn:TButton);
   ...
   begin
    ...
    while not eof(LInf) do
      begin
        Read(LInf,bd[i]);
        with bd[i] do
          M1.Lines.Add(IntToStr(i)+') Цех №'+Nc+', Название: '+nazv+', Количество: '+kol);
        Inc(i);
      end;
   end;
end;
  ...
  if SD.Execute then
   ...
   if SD2.Execute then
     begin
         ...
         Rewrite(TInf);
         for i:=1 to num do
          with bd[i] do writeln(TInf,i:4,'.',' Цех №: ',Nc,', Количество: ',kol,', Название: ',nazv);
         CloseFile(TInf);
     end;
end;
    ...
    begin
      ...
      with bd[1] do
        begin
          ...
          nazv:=E2.Text;
          Nc:=E3.Text;
          M1.Lines.Add('№'+IntToStr(num+1)+', Цех №: '+Nc+', Количество: '+kol+', Название: '+nazv);
        end;
      num:=num+1;
    end
  else
    begin
      ...
      with a[num] do
        begin
          ...
          nazv:=E2.Text;
          Nc:=E3.Text;
          M1.Lines.Add('№'+IntToStr(num)+', Цех №: '+Nc+', Количество: '+kol+', Название: '+nazv);
        end;
      GetMem(bd,num*sizeof(TInfo));
      for i:=1 to num do
        bd[i]:=a[i];
...
procedure TSaS.PLineSearch;
  ...
  begin
    ...
    for i:=1 to num do
      if bd[i].nc = k then
        with bd[i] do
         M1.Lines.Add('Цех №: '+Nc+', Количество: '+kol+', Название: '+nazv);
  end;
