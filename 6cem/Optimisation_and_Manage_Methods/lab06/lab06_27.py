...
def function_f_x(x, B, c):
    return 0.5 * B.T.dot(B).dot(x) + c
...
def analyse(B0, c0, B, c, alpha, x):
    ...
    df = function_f_x(x, B0, c0)
    dg_I0 = array([function_f_x(x, B[i], c[i]) for i in I0])
    # границы
    d_m = zeros(len(x))  # dm
    ...
    f = l.fun
    l = l.x  # l0
    print "План не оптимален." if f < 0 else exit()
    ...
    while True:
        x_t = x + t * (l + a * delta_x)
        _g = array([function_g_x(x_t, B_i, c_i, alpha_i) for B_i, c_i, alpha_i in zip(B, c, alpha)])
    ...
    print "Улучшение плана: ", 100 - function_g_x(x_t, B0, c0, 0.0) / function_g_x(x, B0, c0, 0.0) * 100
