...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		switch (wmId)
		{
		case 150: //add to listbox1
			GetWindowText(textbox, text, 100);  //get text from editbox to variable
			if (check(listbox1, text)) // if text does not exist in listbox -> add string to list
				SendMessage(listbox1, LB_ADDSTRING, NULL, (LPARAM)text);
			break;
		case 151://send to right
			if (int j = SendMessage(listbox1, LB_GETSELITEMS, sizeof(itemarea), (LPARAM)itemarea)) { // if item selected
				SendMessage(listbox1, LB_GETTEXT, itemarea[0], (LPARAM)text); // get selected text to variable to mask array
				if (check(listbox2, text)) // if text does not exist in listbox -> add string to right list
