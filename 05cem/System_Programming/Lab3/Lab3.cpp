...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hButton1 = CreateWindowEx(WS_EX_APPWINDOW
		...
		, 0, 0, 128, 64
		, hWnd, (HMENU)1, hInstance, NULL);        //id 1

	hButton2 = CreateWindowEx(WS_EX_APPWINDOW
		, L"BUTTON", L"TEST"
		, WS_CHILD | WS_VISIBLE | BS_OWNERDRAW
		, 500, 0, 128, 64
		, hWnd, (HMENU)2, hInstance, NULL);        //id 2
...
void DrawButton(LPDRAWITEMSTRUCT dis) {
	...
	if (dis->itemState & ODS_HOTLIGHT)
		hBitmap = hbButtonHover;// load hover bitmap
	if (dis->itemState & ODS_SELECTED)
		hBitmap = hbButtonPressed; // load pressed bitmap
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DRAWITEM: {
		...
		DrawButton(dis);//loaded for each button
		break;
					  }
	case WM_COMMAND:
		wmId    = LOWORD(wParam);
